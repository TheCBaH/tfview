...
The ATen real-kernel build (C++ sources + scripts now in `lib/aten/`, moved out of `modules/` which is submodules-only; see [[aten-static-dispatch]]) compiles with **clang++** and links with **lld** (`-fuse-ld=lld`). This is a hard requirement, not a preference — the user directed it after we hit the blocker.
...
**How to apply:**
- `lib/aten/build_archive.sh`: `clang++` for all compiles; `clang++ -shared -fuse-ld=lld -Wl,--gc-sections` for `dllaten_core.so`.
